...
int isValidEmail(char *email)
{
    ...
    for (i=0; i<Len; i++)
    {
        ...
        if (email[i] == '.')
        {
            if (flagdot && flagdot > flagat)
                return 0;
            flagdot = i;
        }
    }
    if (flagdot && (flagdot - flagat) <= 1)
        return 0;
    if (flagdot == Len)
        return 0;
    if (flagdot && flagat)
...
void Add() //Add a contact
{
    ...
    else
    {
        ...
        while(!isValidAddress(contacts[count].address))
        {
        ...
        phonenumber(count);
        printf("\n\tPerson successfully added to Phonebook\n");
        count++;
    }
}

int searchP(int c) //search for contact
{
    int x,counter=0;
    char a;
    char name[50];
    ...
    if(counter!=0)
        printf("%d contact/s found with this name\n ",counter);
    else
        printf(" No contacts found with this name \n");
...
void Modify()   //Edit contact
{
    ...
    if (y!=0)
    {
        ...
        if (choice==1)
        {
            ...
            scanf("%c",&temp);
            scanf("%[^\n]",contacts[r].address);
            while(!isValidAddress(contacts[r].address))
            {
                printf("Invalid address, Please enter a valid address:");
                scanf("%c",&temp);
                scanf("%[^\n]",contacts[r].address);
